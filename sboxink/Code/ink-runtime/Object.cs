    ...
    public /* TODO: abstract */ class Object
	{
        ...
        public SearchResult ResolvePath(Path path)
        {
            if (path.isRelative) {
                ...
                if (!nearestContainer) {
                    SboxDebug.Assert (this.parent != null, "Can't resolve relative path because we don't have a parent");
                    nearestContainer = this.parent as Container;
                    SboxDebug.Assert (nearestContainer != null, "Expected parent to be a container");
                    SboxDebug.Assert (path.GetComponent(0).isParent);
                    path = path.tail;
                }
